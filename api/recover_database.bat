@echo off
REM =====================================================
REM Bus Management System - Database Recovery Script
REM Purpose: Recover crashed InnoDB tables from .ibd files
REM =====================================================
...
echo ========================================
"%MYSQL_BIN%\mysql.exe" -u %MYSQL_USER% -p < "%SCRIPT_DIR%database_recovery.sql"
if %ERRORLEVEL% NEQ 0 (
...
echo "%MYSQL_BIN%\mysql.exe" -u %MYSQL_USER% -p -e "USE %RECOVERY_DB%; SHOW TABLES; SELECT COUNT(*) FROM users; SELECT COUNT(*) FROM bookings;"
echo.
pause
